...
Open Tabs context bar 在 normal expanded/peek/drawer 状态显示 open-tab count、Enter Selection Mode、Save All 和当前 disclosure command；Save All 包括 eligible pinned tabs。没有 open tab 时 Enter Selection Mode 不可用。Selection Mode 在同一位置替换为 Task 6 批量 actions，并保留当前 disclosure command，不插入第二条 toolbar。Collapsed rail 中同一 bar 只显示居中的 Expand Sidebar，Filter、checkbox、Close 等隐藏 controls 不进入 tab order；collapse 同步 exit + clear selection。Sidebar 底部 Filter tabs 只过滤当前 selected window rows。
